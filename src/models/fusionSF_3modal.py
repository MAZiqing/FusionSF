...
class FusionSF3M(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        print(self.ctx_channels, self.patch_size)
        ...
        self.guide_embedding = nn.Linear(self.guide_channels, dim)
        ...
        if vq_in_ts:
            print('3 modal model, vq in ts')
            self.ts_vq = ResidualVQ(dim=dim, num_quantizers=8, codebook_size=1024,
                                    shared_codebook=True, kmeans_init=True)
        if vq_in_ctx:
            print('3 modal model, vq in ctx')
            self.ctx_vq = ResidualVQ(dim=dim, num_quantizers=8, codebook_size=1024,
                                     shared_codebook=True, kmeans_init=True)
        if vq_in_guide:
            print('3 modal model, vq in guide')
    ...
    def forward(
        ...
        """
        vq_loss = torch.tensor([0.], device=ctx.device, requires_grad=self.training)
        commit_loss = torch.zeros([1, 8], device=ctx.device)
